...
import com.google.mediapipe.examples.gesturerecognizer.databinding.ActivityTailieusoBinding
...
class TaiLieuSoActivity : FragmentActivity() {

    private lateinit var binding: ActivityWithNavBinding
    private lateinit var newsBinding: ActivityTailieusoBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        newsBinding = ActivityTailieusoBinding.inflate(layoutInflater, binding.container, true)
    ...
    private fun fetchVideoData() {
        ...
        val request = Request.Builder()
            .url("https://boxgateway.kozow.com/video/so")
            .addHeader("Authorization", "Bearer $authToken")
            .build()

        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e("TaiLieuSoActivity", "API request failed: ${e.message}")
            }
                ...
                response.use {
                    if (response.isSuccessful) {
                        ...
                        try {
                            ...
                            Log.e("TaiLieuSoActivity", "JSON parsing error: ${e.message}")
                        }
                    } else {
                        Log.e("TaiLieuSoActivity", "API request unsuccessful: ${response.code}")
                    }
                }
            }
        })
    ...
    private fun sendFavoriteVideoToServer(videoId: Int, add: Boolean, callback: (Boolean) -> Unit) {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                runOnUiThread {
                    Toast.makeText(this@TaiLieuSoActivity, "Failed to update favorites: ${e.message}", Toast.LENGTH_SHORT).show()
                }
                Log.e("TaiLieuSoActivity", "Failed to update favorites: ${e.message}")
                callback(false)
            }
                ...
                response.use {
                    if (response.isSuccessful) {
                        runOnUiThread {
                            ...
                            Toast.makeText(this@TaiLieuSoActivity, message, Toast.LENGTH_SHORT).show()
                        }
                        callback(true)
                    } else {
                        runOnUiThread {
                            Toast.makeText(this@TaiLieuSoActivity, "Error: ${response.code}", Toast.LENGTH_SHORT).show()
                        }
                        Log.e("TaiLieuSoActivity", "Failed to update favorites: ${response.code}")
                        callback(false)
                    }
                }
            }
    ...
    private inner class NewsViewHolder(
        ...
        fun bind(videoItem: VideoItem) {
            ...
            Glide.with(this@TaiLieuSoActivity)
