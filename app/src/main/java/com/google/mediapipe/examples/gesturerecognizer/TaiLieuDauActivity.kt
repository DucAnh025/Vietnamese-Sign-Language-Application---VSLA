...
import com.google.mediapipe.examples.gesturerecognizer.databinding.ActivityTailieudauBinding
...
class TaiLieuDauActivity : FragmentActivity() {

    private lateinit var binding: ActivityWithNavBinding
    private lateinit var newsBinding: ActivityTailieudauBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        newsBinding = ActivityTailieudauBinding.inflate(layoutInflater, binding.container, true)
    ...
    private fun fetchVideoData() {
        ...
        val request = Request.Builder()
            .url("https://boxgateway.kozow.com/video/dau")
            .addHeader("Authorization", "Bearer $authToken")
            .build()

        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e("TaiLieuDauActivity", "API request failed: ${e.message}")
            }
                ...
                response.use {
                    if (response.isSuccessful) {
                        ...
                        try {
                            ...
                            Log.e("TaiLieuDauActivity", "JSON parsing error: ${e.message}")
                        }
                    } else {
                        Log.e("TaiLieuDauActivity", "API request unsuccessful: ${response.code}")
                    }
                }
            }
        })
    ...
    private fun sendFavoriteVideoToServer(videoId: Int, add: Boolean, callback: (Boolean) -> Unit) {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                runOnUiThread {
                    Toast.makeText(this@TaiLieuDauActivity, "Failed to update favorites: ${e.message}", Toast.LENGTH_SHORT).show()
                }
                Log.e("TaiLieuDauActivity", "Failed to update favorites: ${e.message}")
                callback(false)
            }
                ...
                response.use {
                    if (response.isSuccessful) {
                        runOnUiThread {
                            ...
                            Toast.makeText(this@TaiLieuDauActivity, message, Toast.LENGTH_SHORT).show()
                        }
                        callback(true)
                    } else {
                        runOnUiThread {
                            Toast.makeText(this@TaiLieuDauActivity, "Error: ${response.code}", Toast.LENGTH_SHORT).show()
                        }
                        Log.e("TaiLieuDauActivity", "Failed to update favorites: ${response.code}")
                        callback(false)
                    }
                }
            }
    ...
    private inner class NewsViewHolder(
        ...
        fun bind(videoItem: VideoItem) {
            ...
            Glide.with(this@TaiLieuDauActivity)
